...
bool create_auto_start_task_for_this_user(bool runEvelvated)
{
    ...
    hr = pService->GetFolder(_bstr_t(L"\\FancyZonesPlus"), &pTaskFolder);
    if (FAILED(hr))
    {
        ...
        hr = pRootFolder->CreateFolder(_bstr_t(L"\\FancyZonesPlus"), _variant_t(L""), &pTaskFolder);
        if (FAILED(hr))
        {
            pRootFolder->Release();
            ExitOnFailure(hr, "Cannot create FancyZones+ task folder: %x", hr);
        }
    }
